...
\end{equation}
%%%
$N_{uc}$ is the number of unit cells, and $c^\mu$ is the concentration of the mixing and the term $g_2(b)$ is a coupling term which defines the strength of the mass disordering
%%%
...
\end{equation}
%%%
with $m^\mu(b)$ being the mass of the $\mu$-th species and $\bar{m}^{\mu}$ is the average mass. There are two atom types in the unit cell, $\mu=a,b$, such that the alloying atom labeled by $m^b_{1-c}$ can be considered to be an ``isotope'' of atom labeled $m^a_{c}$. $g_2(b)$ was set to be zero if atom $b$ remained unmixed (for atoms that do not reside within one monolayer of the interface) since $m^{\mu}(b)=\bar{m}(b)$. The delta function in Eq~\ref{EQ:tau_d} is broadened into a Lorenztian of width on the order of the frequency level spacing. The effective lifetimes obtained from Eq~\ref{EQ:tau_eff} are compared to the lifetimes from NMD upon the interfacially mized superlattices.
