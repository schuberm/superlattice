...
GK.Lx = 18.7587 ; GK.Ly = 9.3794; GK.Lz = 9.3794;
...
plot(GK.JJ(:,1,1),GK.intJJ(:)./max(GK.intJJ(:)))
hold on
plot(GK.JJ(:,1,1),GK.JJ(:,2,1)./max(GK.JJ(:,2,1)))
max(GK.intJJ(:))
pause
...
str.write = strcat(str.main,'/1p_IP_GK.dat');
output = [GK.kappa]
