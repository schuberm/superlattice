...
for i=1:1:length(period)
    str_freq=strcat(num2str(period(i)),'_12p_freq.dat');
    str_vel=strcat(num2str(period(i)),'_12p_vel.dat');
    str_x0=strcat(num2str(period(i)),'_12p_x0.dat');
    str_life=strcat(num2str(period(i)),'_12p_life.dat');
    [f,ip,cp]=omegabin(str_freq,str_vel,str_life,str_x0);
    
    legstr{i}=strcat(num2str(period(i)),'x6x6');
    h(i)=semilogx(f,cp,colour(i),'LineWidth',2)
    hold on
    semilogx(f,ip,colour(i),'LineWidth',2);
...
xlabel('Phonon Mean Free Path $$[m]$$','interpreter','latex','FontSize',14)
